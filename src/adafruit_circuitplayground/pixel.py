...
class Pixel:
    ...
    def __extract_pixel_value(self, val):
        ...
        if len(rgb_value) != 3 or any(not self.__valid_rgb_value(pix) for pix in rgb_value):
            raise ValueError(
                'The pixel color value should be a tuple with three values between 0 and 255 or an hexadecimal color between 0x000000 and 0xFFFFFF.')
    ...
    def brightness(self, brightness):
        if not self.__valid_brightness(brightness):
            raise ValueError(
                'The brightness value should be a number between 0 and 1.')
